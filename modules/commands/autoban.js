...
module.exports.handleEvent = async ({ Users, api, event}) => {
  ...
  const so_lan_spam = 4; // số lần spam, vượt quá sẽ bị ban
  const thoi_gian_spam = 100000; // 60000 millisecond (1 phút)
  const unbanAfter = 3000000; // 600000 millisecond (10 phút) 
	...
	if ((global.client.autoban[senderID].timeStart + thoi_gian_spam) <= Date.now()) {
	  ...
	  if (global.client.autoban[senderID].number >= so_lan_spam) {
	    const time = moment.tz("Asia/Ho_Chi_minh").format("DD/MM/YYYY HH:mm:ss");
			...
			data.banned = true;
			data.reason = `⚔️spam bot ${so_lan_spam} lần/${thoi_gian_spam/100000}phút`;
			data.autoban = {
			  timeStart: Date.now(),
			  unbanAfter
			};
