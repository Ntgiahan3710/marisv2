 ...
 const timee = 10 // trong thời gian `timee` spam `num` lần sẽ bị ban
...
module.exports.handleEvent = async function ({ Users, Threads, api, event})  {
  ...
  if ((global.client.autoban[senderID].timeStart + (timee*1000)) <= Date.now()) {
    ...
    if (global.client.autoban[senderID].number >= num) {
      ...
      api.sendMessage("♥️Bạn đã bị cấm sử dụng bot/♥️ID: " + senderID + " \n♥️Tên: " + dataUser.name + `\n♥️Lý do: spam bot ${num} lần/${timee}s\n\n🔥🔥🔥Đã báo cáo đến admin cute hay dỗi🔥🔥`, threadID,
    () => {
