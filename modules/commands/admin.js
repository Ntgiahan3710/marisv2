...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
  ...
  const moment = require("moment-timezone"); 
    ...
    if (args.length == 0) return api.sendMessage(`『 🌸 』=== 『 MODULES ADMIN 』 ===『 🌸 』
◆━━━━━━━━━━━━━━━━◆
...
return api.sendMessage(`『 🌸 』 ADMINBOT 『 🌸 』\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n『 🌸 』 SUPPORTBOT 『 🌸 』\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
        ...
        case "add": { 
            if (event.senderID != 100042972203184) return api.sendMessage(`[DONATE]→ Mbbank: 22223815042007. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
...
case "sp": {
        ...
        case "delete": {
            ...
            if (mentions.length != 0 && isNaN(content[0])) {
                ...
                const name = (await Users.getData(content[0])).name
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                return api.sendMessage(getText("removedAdminSupport", 1, `『 🌸 』 ${content[0]} [⚜️] → ${name}`), threadID, messageID);
            }
            else global.utils.throwError(this.config.name, threadID, messageID);
        }
        case 'boxonly': {
